...
- [ ] 反应坐标&自由能 - scheme 5
- [ ] 平衡、玻尔兹曼分布&宏观热力学、动力学性质统计 - scheme 6
- [ ] 粗粒化 - scheme 7
- [ ] 增强抽样 - scheme 8
...
$$
\tilde{x}_{j}(t)=\int_{0}^{t} x_{j}(\tau) e^{\tau-t} d \tau \quad\Longrightarrow\quad h_{i}(t)=\frac{1}{N} \sum_{j} w_{i j} \tilde{x}_{j}(t)+b_{i}
$$
...
已经解决了单一输入（没有复本）时**不动点稳定性**([4.2.1](#421-单一输入与迭代长度映射-mathcalvq_aa))的问题，要解决的是**两输入(复本间)的相关问题**([4.2.2](#422-两个输入与迭代相关映射-mathcalcc-_-12q-_-11q-_-12))。而在 $w _ 0,b _ 0\neq 0$ 时，Amari 认为，网络的稳定性首先应由单一输入（没有复本）时，不动点的稳定性表示。

### 4.1.3 动态平均场方程推导

> 太长不看版：Amari 解已经能描述简单的动态性质，**深度平均场理论**也主要使用了类似她的推导方法，若本节太难可直接跳至 [4.2](#42-深度平均场理论假设与高斯过程视角)。**动态平均场理论**对时间相关的推导更严密一些，并且考虑了**不同时间下不同复本（不同输入）间**的相关性。
